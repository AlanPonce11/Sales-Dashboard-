...
import duckdb  # For executing SQL queries on DataFrame
...
# Create a file uploader in the sidebar for configuration
with st.sidebar:
    st.header("Configuration")
    uploaded_file = st.file_uploader("Choose a file")

# If no file is uploaded, display an info message and stop execution
if uploaded_file is None:
    st.info(" Upload a file through config", icon="ℹ️")
    st.stop()

# Function to load data from the uploaded file with caching
@st.cache_data
...
# Display a data preview in an expander
...
def plot_metric(label, value, prefix="", suffix="", show_graph=False, color_graph=""):
    ...
    # Add a line chart if show_graph is True
    if show_graph:
        fig.add_trace(
            go.Scatter(
...
# Function to plot the top-right chart
def plot_top_right():
    # SQL query to fetch data for the top-right chart
...
# Function to plot the bottom-left chart
def plot_bottom_left():
    # SQL query to fetch data for the bottom-left chart
    sales_data = duckdb.sql(
        f"""
        WITH sales_data AS (
            SELECT 
...
# Function to plot the bottom-right chart
def plot_bottom_right():
    # SQL query to fetch data for the bottom-right chart
    sales_data = duckdb.sql(
        f"""
        WITH sales_data AS (
            UNPIVOT ( 
...
with top_left_column:
    ...
    # Metrics and gauges in the top-left quadrant
    with column_1:
        plot_metric(
